	...
	public boolean saveToDB(SQLiteStore SQLS) {
		try {
			for (int d = 0; d < this.directors.size(); d++) {
				SQLS.execQuery("INSERT INTO directors (serieId, episodeId, director) VALUES ('"+ this.seriesId
					+"', '"+ this.id +"', "+ DatabaseUtils.sqlEscapeString(this.directors.get(d)) +");");
			}
			for (int g = 0; g < this.guestStars.size(); g++) {
				SQLS.execQuery("INSERT INTO guestStars (serieId, episodeId, guestStar) VALUES ('"+ this.seriesId
					+"', '"+ this.id +"', "+ DatabaseUtils.sqlEscapeString(this.guestStars.get(g)) +");");
			}
			for (int w = 0; w < this.writers.size(); w++) {
				SQLS.execQuery("INSERT INTO writers (serieId, episodeId, writer) VALUES ('"+ this.seriesId
					+"', '"+ this.id +"', "+ DatabaseUtils.sqlEscapeString(this.writers.get(w)) +");");
			}
			int iseen = 0;
			if (this.seen) {
				iseen = 1;
			...
			+"absoluteNumber, filename, lastUpdated, seasonId, seen) VALUES ('"+ this.seriesId
			+"', '"+ this.id +"', '"+ this.combinedEpisodeNumber +"', '"+ this.combinedSeason
			+"', '"+ this.dvdChapter +"', '"+ this.dvdDiscId +"', '"+ this.dvdEpisodeNumber
			+"', '"+ this.dvdSeason +"', '"+ this.epImgFlag +"', "+ DatabaseUtils.sqlEscapeString(this.episodeName)
			+", " + this.episodeNumber +", '"+ this.firstAired +"', '"+ this.imdbId +"', '"+ this.language
			+"', "+ DatabaseUtils.sqlEscapeString(this.overview) +", '"+ this.productionCode +"', '"+ this.rating
